...
→ Tokenは**お使いのブラウザにのみ保存**され、加えて **管理パスワードで暗号化されたコピーがリポジトリ内（admin-vault.js）にも保存** されます。

### 🔄 複数PCでの利用（自動同期）
1台目のPCでTokenを保存すると、暗号化された状態でリポジトリ内に保存されます（admin-vault.js）。

**2台目以降のPCでは、管理パスワード（Kamagaya1123）でログインするだけでTokenが自動取り込みされます。** GitHubのToken発行作業は1度だけでOK。

- 暗号化：PBKDF2(SHA-256, 100,000回) + AES-GCM 256bit
- 復号には管理パスワードが必須（パスワードを知らないとTokenは取り出せない）

### Tokenを削除したい場合
管理画面の「削除」ボタンを押すと、削除範囲を聞かれます：
- **「1」**：このPCのlocalStorageからのみ削除（他PCはそのまま使用可能）
- **「2」**：vault（リポジトリ内）から削除 → 全PCのTokenが無効化される（再発行→再保存が必要）
